...
// V5: Goal is to make program work completely
...
int main(){
	...
	//for(int i = 0; i < NumOfFiles; i++){
		...
		if(inFile.is_open()){
			...
			cout<<setw(10)<<"Mdis"<<setw(15)<<"rho"<<setw(15)<<"pressure"<<setw(15)<<"vel_km_per_s"<<setw(15)<<"Temp"<<endl;
			for (int a = 0; a < NumOfZones; a++){
				
				inFile >> Mdis[a] >> rho[a] >> pressure[a] >> vel_km_per_s[a] >> Temp[a];
				cout<<setw(10)<<Mdis[a]<<setw(15)<<rho[a]<<setw(15)<<pressure[a]<<setw(15)<<vel_km_per_s[a]<<setw(15)<<Temp[a]<<endl;
